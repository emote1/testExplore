  ...
  await query(
    `INSERT INTO verified_contract (id, name, type, contract_data, timestamp)
     ...
     ON CONFLICT (id) DO UPDATE SET
       type = CASE
         WHEN verified_contract.type = 'Contract' AND EXCLUDED.type IN ('ERC20', 'ERC721', 'ERC1155')
           THEN EXCLUDED.type
         WHEN verified_contract.type = 'ERC20' AND EXCLUDED.type IN ('ERC721', 'ERC1155')
           THEN EXCLUDED.type
         ELSE verified_contract.type
       END,
       name = CASE
         WHEN EXCLUDED.name IS NULL OR btrim(EXCLUDED.name) = ''
           THEN verified_contract.name
         WHEN verified_contract.name IS NULL OR btrim(verified_contract.name) = ''
           THEN EXCLUDED.name
         WHEN verified_contract.name ~* '^(Contract|ERC20|ERC721|ERC1155)-0x[0-9a-f]{6,}$'
           THEN EXCLUDED.name
         WHEN lower(verified_contract.name) IN ('unknown', 'token')
           THEN EXCLUDED.name
         WHEN verified_contract.type = 'Contract' AND EXCLUDED.type IN ('ERC20', 'ERC721', 'ERC1155')
           THEN EXCLUDED.name
         WHEN verified_contract.type = 'ERC20' AND EXCLUDED.type IN ('ERC721', 'ERC1155')
       ...
       END,
       contract_data = CASE
         WHEN EXCLUDED.contract_data IS NOT NULL AND verified_contract.contract_data IS NULL
           THEN EXCLUDED.contract_data
         ELSE verified_contract.contract_data
       END`,
...
// ─── Icon enrichment helpers ─────────────────────────────────

/** Get verified contract IDs that have no iconUrl in contract_data */
export async function getContractsWithoutIcon(): Promise<string[]> {
  const res = await query(
    `SELECT id FROM verified_contract
     WHERE type IN ('ERC20', 'ERC721', 'ERC1155')
       AND (contract_data IS NULL
            OR NOT (contract_data::jsonb ? 'iconUrl'))
     ORDER BY timestamp DESC NULLS LAST`
  );
  return res.rows.map((r: { id: string }) => r.id);
}

/** Merge iconUrl into existing contract_data (JSONB merge) */
export async function mergeContractIcon(id: string, iconUrl: string): Promise<void> {
  await query(
    `UPDATE verified_contract
